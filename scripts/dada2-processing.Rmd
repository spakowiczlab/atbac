...
taxa <- assignTaxonomy(seqtab.nochim, "../../../db/dada2/rdp_train_set_14.fa.gz")
unname(head(taxa))
```

# Test sequence lengths, filter
```{r}
l <- nchar(colnames(seqtab.nochim))
...
```{r}
longenough <- l > 435 & l < 470
sum(longenough) / length(longenough)

seqtabNoCf <- seqtab.nochim[, longenough]
taxf <- taxa[longenough, ]
```

# Save filtered tables
```{r}
saveRDS(seqtabNoCf, file = "../data/seqtabNoCf_update.Rdata")
saveRDS(taxf, file = "../data/taxf_update.Rdata")
```
