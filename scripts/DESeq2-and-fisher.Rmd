...
```{r}
seqtab <- readRDS("../data/seqtabNoCf.RDS")
tax <- readRDS("../data/taxf.RDS")
...
seqtab.labs.r <- seqtab.ncon.r %>%
  ...
  left_join(tax) %>%
  separate(sample, into = c("patient", "samptag"), by = "-") %>%
...
dds.res.r <- lapply(c("Phylum", "Class", "Order", "Family", "Genus"), function(x) labs2DESeqLevs(seqtab.labs.r, x)) %>%
  bind_rows() %>%
  mutate(padj = p.adjust(pvalue, method = "fdr")) %>%
  arrange(pvalue)
```
