...
```{r}
taxcounts.nocontam <- readRDS("../data/taxcounts_interface-DNA_no-neg-contams.RDS")

taxcounts.nocontam.phyl <- taxcounts.nocontam %>%
  mutate(Phylum = as.character(Phylum),
         Phylum = ifelse(is.na(Phylum), "Unclassified", Phylum)) %>%
  group_by(sample, Phylum) %>%
...
getsampord <- bmi.stat %>%
  ...
  left_join(taxcounts.nocontam.phyl) %>%
...
taxcounts.nocontam.phyl %>%
  ...
  ggplot(aes(x = fct_relevel(sample, rev(getsampord$sample)), 
             y = count, 
  ...
  ggsave("../figures/stacked-bar_phyla_DNA-ordered_nocontam.pdf", width = 10)
```

# Sourcetracker Visualization

```{r}
sourceres <- read.table("../data/mixing_proportions.txt",
                        sep = "\t", header = T) 
...
sourceres  %>%
  ...
  # filter(source != "Other") %>%
  ggplot(aes(x = fct_relevel(SampleID, rev(getsampord$sample)), 
             y = prop, 
             fill = source)) +
  geom_col(position = "fill") +
  ...
  theme_bw() +
  theme(axis.text.x = element_blank()) +
  ggsave("../figures/stacked-bar_sourcetracker_DNA-ordered_nocontam.pdf", width = 10)
