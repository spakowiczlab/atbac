...
taxcounts.phyl <- taxcounts %>%
  mutate(Phylum = ifelse(is.na(Phylum), paste("Unclassified", Kingdom), Phylum)) %>%
...
phyls <- unique(taxcounts.phyl$Phylum)
...
taxcounts.phyl %>%
  ...
  geom_col(position = "fill") +
  scale_fill_manual(breaks = phyls, values = getPalette(length(phyls))) + 
  labs(x = "", y = "") +
  theme_bw() +
