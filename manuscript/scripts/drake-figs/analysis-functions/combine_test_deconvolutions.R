combine_test_deconvolutions <- function(){
  ...
  blood.frc1 <- Blood1 %>%
    ...
    tidyr::gather(-Input.Sample, key = "celltype", value = "fraction") %>% #gather the colomns and collapse them into Celltypes and fraction
  ...
  blood.frc2 <- Blood2 %>%
    ...
    tidyr::gather(-Input.Sample, key = "celltype", value = "fraction") %>%
  ...
  bardat <- all.frc %>%
    tidyr::gather(-Input.Sample, -source, key = "celltype", value = "predfrac")
