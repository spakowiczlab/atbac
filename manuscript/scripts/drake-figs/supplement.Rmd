...
distmat %>%
  mutate(timepoint = fct_relevel(timepoint, "Pre", "Post"),
         diet.group = gsub("gavage", " gavage,", diet.group)) %>%
  ggplot(aes(x = timepoint, y = distance, fill = diet.group)) +
  geom_boxplot() +
  labs(x = "Time point", y = "Gower's distance") +
  scale_fill_brewer(palette = "Dark2", name = "Diet group",
                    na.value = "grey") +
  theme_bw()
ggsave("../../figures/boxplot_zymo_distance-from-gavage.png")
```

# Boxplots absolute abundance

```{r}
zymo.abun.abs <- read.csv("../../data/zr8258.16S_220923.zymo/00...AllSamples.Bac16Sv34/Sample_Information/absolute.abundance.csv")
...
stool.samps %>%
  ggplot(aes(x = timepoint, y = gene_copies_per_ul)) +
  geom_boxplot() +
  scale_x_discrete(breaks = c("Pre", "Pos", "End"),
          labels = c("Pre-antibiotic", "Post-antibiotic", 
                     "Study end")) +
  labs(x = "Time point", y = "Gene copies per ul sample") +
  theme_bw()
