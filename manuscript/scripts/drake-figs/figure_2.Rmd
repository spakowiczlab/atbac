...
# B0: Addt. sample order label

```{r}
load("prepared-data/sample-bmi-firmicutes.rda")
...
# B1: Stacked bar: Phyla level bacteria

```{r}
load("prepared-data/sample-bmi-firmicutes.rda")
load("prepared-data/taxcounts-nocontam-phyl.rda")
...
# C: Stacked bar: Sourcetracker results

```{r}
load("prepared-data/sample-bmi-firmicutes.rda")
load("prepared-data/sourceres.rda")
sourceres  %>%
...
#   select(-Kingdom) %>%
#   pivot_longer(-"Sequence", names_to = "Taxa.level", values_to = "Taxa") %>%
#   drop_na() %>%
#   select(-Sequence) %>%
#   mutate(proteobacteria = "Proteobacteria")
```

```{r}
load("prepared-data/bacteria-desres.rda")
bacteria.desres %>%
  mutate(colgroup = ifelse(pvalue < 0.1, Taxa.Level, NA)
         # lab.plotmath = ifelse(!is.na(colgroup),
         #                       ifelse(Taxa.Level == "Genus", paste0("italic(", Taxa, ")"), Taxa),
         #                       NA)
         ) %>%
  ...
  geom_vline(xintercept = 2, linetype = "dashed") +
  scale_color_brewer(palette = "Dark2", na.value = "grey50", name = "Taxonomy\nLevel") +
  labs(x = "log(fold change, base = 2)", y = "-log(p value)") +
  theme_bw(base_size = 9) +
  theme(axis.title = element_text(family = "Times New Roman"))
# theme(legend.position = c(0.15, 0.2),
#       legend.box.background = element_rect(color = "black"))
ggsave("../../figures/volcano_deseq2-DNA-results.svg",height = 2.5, width = 3.5) # Must italics in illustrator due to length requiring repel
ggsave("../../figures/volcano_deseq2-DNA-results.png",
       height = 2.5, width = 3.5, dpi = 300)


# bacteria.desres %>%
#   left_join(proteotax) %>%
#   mutate(proteobacteria = replace_na(proteobacteria, "Not Proteobacteria")) %>%
#   mutate(colgroup = ifelse(pvalue < 0.1, Taxa.Level, NA)) %>%
#   ggplot(aes(x = log2FoldChange, y = -log(pvalue), color = colgroup, shape = proteobacteria)) +
#   geom_point() +
#   geom_text_repel(aes(label = ifelse(!is.na(colgroup), Taxa, NA)),
#                   size = 3, 
#                   show.legend = FALSE) +
...
# ggsave("../../figures/volcano_deseq2-DNA-results_proteobacteria.png",
