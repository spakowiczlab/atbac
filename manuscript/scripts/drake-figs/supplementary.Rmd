...
flextable(accession.table, cwidth = 2, cheight = 1.25) %>%
save_as_docx(path = "../../tables/table-s2.docx")
```
...
bind_rows(blood.accession, vat.accession) %>%
  flextable(cwidth = 1) %>%
save_as_docx(path = "../../tables/table-s4.docx")
```
...
```{r}
# Compare End and Post timepoints
kruskal.test(distance ~ timepoint, data = filter(distmat, timepoint != "Pre"))

# Compare End and Pre timepoints
kruskal.test(distance ~ timepoint, data = filter(distmat, timepoint != "Post"))
```
...
```{r}
# Compare End and Post timepoints
kruskal.test(gene_copies_per_ul ~ timepoint, data = filter(stool.samps, timepoint != "Pre"))

# Compare End and Pre timepoints
kruskal.test(gene_copies_per_ul ~ timepoint, data = filter(stool.samps, timepoint != "End"))
```