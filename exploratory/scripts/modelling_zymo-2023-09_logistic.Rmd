...
tax.form <- tax %>%
  ...
  mutate(phylum = ifelse(phylum == "p__NA", paste0("p__unclassified-", kingdom), phylum),
         ...
         species = ifelse(species == "s__NA", paste0("s__unclassified-", gsub("g__unclassified-", "", genus)), species))
...
rangedat.form <- rangedat %>%
  mutate(direction = ifelse(log2fc < 0, "Lean", "Obese"),
         ...
         patval = ifelse(direction == "Lean", patnum - .1, patnum + .1),
...
rangedat.form %>%
  drop_na(direction) %>%
  ggplot(aes(x = patval, y = -log(p.value), size = sizeval, color =direction)) +
  geom_point() +
