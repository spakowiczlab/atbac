...
calculateAvgZScore <- function(gene_matrix, genes){
  ...
  avg_z_score <- gene_matrix %>%
    tidyr::gather(-Gene.Symbol, key = "sample", value = "counts") %>%
...
scrna.sig.score <- calculateAvgZScore(scrna.frmt, neutmarkers) %>%
  rename(sig_score = avg_z_score) %>%
  left_join(select(scrna.data, c('Cell Type', umap_1, umap_2)), by = rownames(scrna.data))
```
